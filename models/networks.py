...
def define_net_recon(net_recon, use_last_fc=False, init_path=None, model='facerecon'):
    return ReconNetWrapper(net_recon, use_last_fc=use_last_fc, init_path=init_path, model=model)
...
class ReconNetWrapper(nn.Module):
    fc_dim=257
    def __init__(self, net_recon, use_last_fc=False, init_path=None, model="facerecon"):
        super(ReconNetWrapper, self).__init__()
        self.use_last_fc = use_last_fc
        if net_recon not in func_dict:
            return  NotImplementedError('network [%s] is not implemented', net_recon)
        if model=='facereconfv':
            self.fc_dim=508
        ...
        if not use_last_fc:
            ...
            elif self.fc_dim==508:
                self.final_layers = nn.ModuleList([
                    conv1x1(last_dim, 150, bias=True), # id layer
                    conv1x1(last_dim, 74, bias=True),  # exp layer
