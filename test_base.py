...
def main(rank, opt, name='examples'):
    ...
    for i in range(len(im_path)):
        ...
        # shape,tex=model.get_uv()
        # uv_texture = tex[0].permute(1,2,0).detach().cpu().numpy() * 255
        # cv2.imwrite((os.path.join(visualizer.img_dir, name.split(os.path.sep)[-1], 'epoch_%s_%06d' % (opt.epoch, 0),img_name + '_uvtex.jpg')), uv_texture[:,:,::-1])
        #
        # vers=model.pred_vertex.cpu().numpy()[0]
        # np.save((os.path.join(visualizer.img_dir, name.split(os.path.sep)[-1], 'epoch_%s_%06d' % (opt.epoch, 0),img_name + '.npy')), vers)
