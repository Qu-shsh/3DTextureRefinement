...
def load_lm3d_faceverse(faceverse_folder):

    Lm3D = np.loadtxt(osp.join(faceverse_folder, "std_vertices3.txt")).astype(np.float32) # base
    # Lm3D = np.loadtxt(osp.join(faceverse_folder, "std_vertices_detail.txt")).astype(np.float32) #detail
    Lm3D = Lm3D.reshape([-1, 3])

    lm_idx = np.array([13927, 9432, 9112, 9164, 9527, 21008, 21066]) #base [31, 37, 40, 43, 46, 49, 55]-1
    # lm_idx = np.array([219143, 148907, 142872, 143765, 149965, 330663, 330899])  #detail [31, 37, 40, 43, 46, 49, 55]-1
    Lm3D = np.stack([Lm3D[lm_idx[0], :], np.mean(Lm3D[lm_idx[[1, 2]], :], 0), np.mean(
        Lm3D[lm_idx[[3, 4]], :], 0), Lm3D[lm_idx[5], :], Lm3D[lm_idx[6], :]], axis=0) #[5,3]
    Lm3D = Lm3D[[1, 2, 0, 3, 4], :]
